...
export const SingleMapTrackOrder = props => {
  ...
  const [nextLatLongPosition, setNextLatLongPosition] = useState({ latitude: 0.0, longitude: 0.0 });
  const [previousLatLongPosition, setPreviousLatLongPosition] = useState({ latitude: 0.0, longitude: 0.0 });
  const updateNextLatLongPosition = (data) => setNextLatLongPosition((state) => ({ ...state, ...data }));

  const [currentNewPosition, setNewPosition] = useState({ lat: 0.0, long: 0.0 });
  const [previousPosition, setPreviousPosition] = useState({ lat: 0.0, long: 0.0 });
  ...
  useEffect(() => {
    setPreviousLatLongPosition(nextLatLongPosition);
    updateNextLatLongPosition({
      latitude: stateCoordinates?.response?.driverLatitude,
      longitude: stateCoordinates?.response?.driverLongitude
    })

    const fetchRoute = async () => {
      // const response = await fetch(
      //   `https://maps.googleapis.com/maps/api/directions/json?origin=${stateCoordinates?.response?.pickUpLatitude},${stateCoordinates?.response?.pickUpLongitude}&destination=${stateCoordinates?.response?.dropOffLatitude},${stateCoordinates?.response?.dropOffLongitude}&key=${GOOGLE_MAPS_APIKEY}`
      // );
      console.log("fetchRoute", previousLatLongPosition, nextLatLongPosition);


      const response = await fetch(
        `https://maps.googleapis.com/maps/api/directions/json?origin=${previousLatLongPosition?.latitude},${previousLatLongPosition?.longitude}&destination=${nextLatLongPosition.latitude},${nextLatLongPosition.longitude}&key=${GOOGLE_MAPS_APIKEY}`
      );
      const data = await response.json();
      const points = decodePolyline(data.routes[0].overview_polyline.points);
      setRoute(points);

      calculateBearing(previousLatLongPosition?.latitude, previousLatLongPosition?.longitude, nextLatLongPosition.latitude, nextLatLongPosition.longitude)

      const remainingDistance = calculateDistance(
        markerRef.current.latitude,
        markerRef.current.longitude,
        currentNewPosition.latitude,
        currentNewPosition.longitude
      );
  ...
  }, [stateCoordinates?.response?.driverLatitude, stateCoordinates?.response?.driverLongitude])
  ...
  const animateMarkerUpdate = () => {
    let index = 0;
    const interval = autoRefreshTrackingInterval * 1000; // milliseconds
    const moveMarker = () => {
      if (index < route.length) {
        if (markerRef.current) {
          const nextCoord = route[index];
          //compare live data and nextCoord

          console.log("nextCoordnextCoordnextCoord", nextCoord)
          ...
          markerRef.current.animateMarkerToCoordinate(nextCoord, 3000);
  ...
  const calculateBearing = (lat1, lon1, lat2, lon2) => {
    ...
    //console.log("77777777777777777", distance)
            ...
            {stateCoordinates?.response?.pickUpLatitude && (
              ...
              >
                ...
                {/* <Marker
                  ...
                  <Image
                    //source={require('../../../branding/carter/assets/images/dot-marker.png')}
                    source={require('../../../branding/carter/assets/images/asli-bikee.png')}
                ...
                <MapViewDirections
                  origin={{
                    ...
                    latitude: stateCoordinates?.response?.pickUpLatitude || 0.0,
                    longitude: stateCoordinates?.response?.pickUpLongitude || 0.0,
                  }}
                  destination={{
